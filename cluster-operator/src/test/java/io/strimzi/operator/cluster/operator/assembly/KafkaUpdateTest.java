...
public class KafkaUpdateTest {
    ...
    @Test
    public void upgrade111_200_withEmptyConfig(TestContext context) throws IOException {
        try {
            testUpgrade111_200_messageFormatConfig(context, emptyMap(), false);
    ...
    @Test
    public void upgrade111_200_with111messageFormatConfig(TestContext context) throws IOException {
        testUpgrade111_200_messageFormatConfig(context, singletonMap(LOG_MESSAGE_FORMAT_VERSION, "1.1"), false);
    }

    @Test
    public void upgrade111_200_with100messageFormatConfig(TestContext context) throws IOException {
        testUpgrade111_200_messageFormatConfig(context, singletonMap(LOG_MESSAGE_FORMAT_VERSION, "1.0"), false);
    }

    @Test
    public void upgrade111_200_with111protocolVersion(TestContext context) throws IOException {
        testUpgrade111_200_messageFormatConfig(context,
                (Map) map(LOG_MESSAGE_FORMAT_VERSION, "1.1",
                INTERBROKER_PROTOCOL_VERSION, "1.1"), true);
    }

    @Test
    public void upgrade111_200_with100protocolVersion(TestContext context) throws IOException {
        testUpgrade111_200_messageFormatConfig(context,
                (Map) map(LOG_MESSAGE_FORMAT_VERSION, "1.1",
                        INTERBROKER_PROTOCOL_VERSION, "1.0"), true);
    }

    private void testUpgrade111_200_messageFormatConfig(TestContext context, Map<String, Object> config, boolean expectSinglePhase) throws IOException {
        String initialKafkaVersion = "1.1.1";
        String upgradedKafkaVersion = "2.0.0";
        ...
        if (!expectSinglePhase) {
            ...
            KafkaConfiguration config1 = KafkaConfiguration.unvalidated(ModelUtils.findEnv(env, ENV_VAR_KAFKA_CONFIGURATION).getValue());
            context.assertEquals(config.getOrDefault(INTERBROKER_PROTOCOL_VERSION, "1.1"), config1.getConfigOption(INTERBROKER_PROTOCOL_VERSION));
            context.assertEquals(config.getOrDefault(LOG_MESSAGE_FORMAT_VERSION, "1.1"), config1.getConfigOption(LOG_MESSAGE_FORMAT_VERSION));
        }
        ...
        KafkaConfiguration config2 = KafkaConfiguration.unvalidated(ModelUtils.findEnv(env2, ENV_VAR_KAFKA_CONFIGURATION).getValue());
        context.assertEquals(config.get(INTERBROKER_PROTOCOL_VERSION), config2.getConfigOption(INTERBROKER_PROTOCOL_VERSION));
        context.assertEquals(config.getOrDefault(LOG_MESSAGE_FORMAT_VERSION, "1.1"), config2.getConfigOption(LOG_MESSAGE_FORMAT_VERSION),
    ...
    @Test
    public void testUpgrade111_200_messageFormatConfig_exceptionDuringPhase0Roll(TestContext context) throws IOException {
        Map<String, Object> config = singletonMap(LOG_MESSAGE_FORMAT_VERSION, "1.1.1");
        String initialKafkaVersion = "1.1.1";
        String upgradedKafkaVersion = "2.0.0";
        String upgradedImage = "strimzi/kafka:0.8.0-kafka-" + upgradedKafkaVersion;
        ...
        KafkaConfiguration config1 = KafkaConfiguration.unvalidated(ModelUtils.findEnv(env1, ENV_VAR_KAFKA_CONFIGURATION).getValue());
        context.assertEquals(config.getOrDefault(INTERBROKER_PROTOCOL_VERSION, "1.1"), config1.getConfigOption(INTERBROKER_PROTOCOL_VERSION));
        context.assertEquals(config.getOrDefault(LOG_MESSAGE_FORMAT_VERSION, "1.1"), config1.getConfigOption(LOG_MESSAGE_FORMAT_VERSION));
    ...
    @Test
    public void downgrade111_200_withEmptyConfig(TestContext context) throws IOException {
        try {
            testDowngrade200_111_messageFormatConfig(context, emptyMap(), true);
            context.fail();
        } catch (UpgradeException e) {
            context.assertTrue(e.getCause() instanceof KafkaUpgradeException);
            context.assertTrue(e.states.isEmpty());
    ...
    @Test
    public void downgrade200_111_with200messageFormatConfig(TestContext context) throws IOException {
        try {
            testDowngrade200_111_messageFormatConfig(context, singletonMap(LOG_MESSAGE_FORMAT_VERSION, "2.0"), true);
            context.fail();
        } catch (UpgradeException e) {
            context.assertTrue(e.getCause() instanceof KafkaUpgradeException);
            context.assertTrue(e.states.isEmpty());
    ...
    @Test
    public void downgrade200_111_with111messageFormatConfig(TestContext context) throws IOException {
        testDowngrade200_111_messageFormatConfig(context, singletonMap(LOG_MESSAGE_FORMAT_VERSION, "1.1"), true);
    }

    @Test
    public void downgrade200_111_with200protocolVersion(TestContext context) throws IOException {
        testDowngrade200_111_messageFormatConfig(context,
                (Map) map(LOG_MESSAGE_FORMAT_VERSION, "1.1",
    ...
    @Test
    public void downgrade200_111_with111protocolVersion(TestContext context) throws IOException {
        testDowngrade200_111_messageFormatConfig(context,
                (Map) map(LOG_MESSAGE_FORMAT_VERSION, "1.1",
    ...
    private void testDowngrade200_111_messageFormatConfig(TestContext context, Map<String, Object> config, boolean expectFirstPhase) throws IOException {
        String initialKafkaVersion = "2.0.0";
        String downgradedKafkaVersion = "1.1.1";
