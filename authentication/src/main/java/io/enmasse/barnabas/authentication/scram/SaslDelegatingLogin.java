...
public class SaslDelegatingLogin implements LoginModule {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(SaslDelegatingLogin.class);
    ...
    public boolean commit() throws LoginException {
        ...
        try {
            if (result) {
                ...
                LOG.debug("Adding user principal for: {0}", user);
                for (String entry : roles) {
                    LOG.debug("Adding role principal for: {0}", entry);
            ...
            LOG.debug("commit, result: {0}", result);
    ...
    private void readFromNetwork(Connection connection, InputStream in, BooleanSupplier test) throws IOException, LoginException {
        ...
        while(test.getAsBoolean()) {
            ...
            LOG.trace("read {0} bytes", bytesRead);
            if (bytesRead == -1) {
    ...
    private void writeToNetwork(Connection connection, OutputStream out) throws IOException {
        ...
        while(transport.pending() > 0)
        {
            ...
            LOG.trace("writing {0} bytes", size);
            out.write(tmpBuf);
            transport.pop(size);
        }
    }
